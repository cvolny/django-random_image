...
def get_openshift_secret_token():
    ...
    if token is not None:
        ...
        return hashlib.sha256(name.encode('utf-8') + '-' + uuid.encode('utf-8')).hexdigest()
    return None
...
def openshift_secure(default_keys, secure_function='make_secure_key'):
    ...
    if my_token is not None:
        ...
        for key, value in default_keys.items():
            ...
            sha = hashlib.sha256(my_token.encode('utf-8') + '-' + key.encode('utf-8')).hexdigest()
        ...
        sys.stderr.write(
            "OPENSHIFT WARNING: Using default values for secure variables, please manually modify in "
            + calling_file + "\n")
